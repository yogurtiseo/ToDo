...
# - db: FastAPI가 get_db() 함ㅅ수를 통해 자동으로 주입하는 DB 세션 객체체
async def create_task(
    ...
    #   - 테스트 시에는 get_db 함수를 오버라이드해서 가짜 DBㄴ아 테스트용 DB를 넣을 수 있음

    # * 이 구조는 DB 작업(비즈니스 로직)은 crud.py에 따로 만들고,
    #   이 함수는 요청 받고 응답하는 역할만 담당하도록 나눠서 구성함
    #   -> 코드가 더 깔끔하고 관리하기가 쉬워짐짐
...
async def delete_task(task_id: int):
    return
    # * 실제 구현에서는 삭제 후 상태 코드나 메시지를 반환할 수 있음음
